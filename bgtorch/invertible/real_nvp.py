...
class AffineTransformer(torch.nn.Module):
    def __init__(self, masked_transform, dt=1.0, init_downscale=1.0):
        super().__init__()
    ...
    def forward(self, x, y, inverse=False, dt=None):
        ...
        if mu_log_sigma.shape[-1] != 2 * n_dim_right:
            raise ValueError(
                "`masked_transform` has wrong output dimension:"
                "expected {} but was {}".format(2 * n_dim_right, mu_log_sigma.shape[-1])
            )
...
class RealNVP(torch.nn.Module):
    def __init__(self, transformer, dt=1.0):
        ...
        masked_transform: PyTorch Module
            ...
            to
                `[..., 2 * n_dim_right]`.
        dt : Float > /home/jonas/dev/PhD/bgtorch/bgtorch/invertible/real_nvp.py0
...
class _RealNVP_old(torch.nn.Module):
    def __init__(self, masked_transform, dt=1.0):
        """
        Coupling layer based on the RealNVP design.
    ...
    def forward(self, x_left, x_right, inverse=False, dt=None):
        ...
        if mu_log_sigma.shape[-1] != 2 * n_dim_right:
            raise ValueError(
                "`masked_transform` has wrong output dimension: expected {} but was {}".format(
