...
class MixtureModel(EnergyBasedModel):
    def __init__(
        self, n_dims, n_clusters=5, spread=1, temperature=1.0, excentricity=0.4
    ):
        super().__init__()
        ...
        self._d = torch.Tensor(n_clusters, n_dims).uniform_(
            1.0 - excentricity, 1.0 + excentricity
        )
    ...
    def sample(self, sample_shape, temperature=None):
        ...
        i = np.random.choice(
            self._n_clusters, sample_shape, p=self._weights.numpy()
        ).reshape(-1)
    ...
    def energy(self, x, temperature=None):
        ...
        energy = -torch.logsumexp(
            energy + torch.log(self._weights).unsqueeze(0), dim=-1
        )
