...
class OneParticleTestBridge(OpenMMBridge):
    ...
    def __init__(self, n_workers=1, n_simulation_steps=0):
        ...
        super(OneParticleTestBridge, self).__init__(
            ...
            n_workers=n_workers, n_simulation_steps=n_simulation_steps
        )
...
def test_openmm_bridge_evaluate_dummy(n_workers, n_simulation_steps):
    ...
    if n_simulation_steps == 0:
        assert energies.numpy()[0] == pytest.approx([0.1*unit.kilojoule_per_mole/kT], abs=1e-8, rel=0)
        assert forces.numpy()[0] == pytest.approx([-1.*unit.kilojoule_per_mole/kT,0.,0.], abs=1e-8, rel=0)
    else:
        assert energies.numpy()[0] != pytest.approx([0.1*unit.kilojoule_per_mole/kT], abs=1e-8, rel=0)
...
def test_openmm_bridge_evaluate_openmmtools_testsystem(testsystem_name, batch_size, n_workers):
    ...
    if testsystem_name == "WaterBox" and n_workers == 1: pytest.skip()
