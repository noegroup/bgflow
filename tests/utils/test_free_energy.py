...
@pytest.mark.parametrize("method", ["torch", "pymbar"])
@pytest.mark.parametrize("compute_uncertainty", [True, False])
def test_bar_same_distribution(ctx, method, compute_uncertainty):
    pytest.importorskip("pymbar")
    dim = 1
    energy1 = UniformDistribution(low=torch.zeros(dim, **ctx), high=torch.ones(dim, **ctx), validate_args=False)
    energy2 = UniformDistribution(low=torch.zeros(dim, **ctx), high=torch.ones(dim, **ctx), validate_args=False)
    samples1 = energy1.sample(10000)
    samples2 = energy2.sample(20000)
