    ...
    def _step(self, state: SamplerState):
        ...
        if state.box_vectors is not None:
            assert torch.allclose(
                torch.tril(state.box_vectors[0], diagonal=-1),
                torch.zeros_like(state.box_vectors[0])
        ...
        state.box_vectors = (
            ...
            else pack_tensor_in_list(openmm_output.new_box_vectors)
        )
...
class ApplyPeriodicBoundariesStep(SamplerStep):
    ...
    def _step(self, state):
        ...
        if state.box_vectors is None:
            warnings.warn("Sampler state has no box vectors. Skipping ApplyPeriodicBoundariesStep.", UserWarning)
        a,b,c = state.box_vectors[...]  #TODO
